...
class VideoPlayerDialog(QDialog):
    def __init__(self, video_path: str, parent=None):
        super().__init__(parent)
        self.setWindowTitle(f"HomeGuard Player — {Path(video_path).name}")
...
class GalleryWidget(QWidget):
    ...
    def _build(self) -> None:
        ...
        toolbar = QHBoxLayout()
        title = QLabel("📹  Recorded Alerts")
        title.setStyleSheet("font-size: 15px; font-weight: bold; color: #c0c0f0;")
    ...
    def refresh(self) -> None:
        ...
        if not os.path.exists(self.recordings_dir):
            self.count_lbl.setText("No recordings folder")
            return
        ...
        if not files:
            self.count_lbl.setText("No recordings yet")
            placeholder = QLabel("No recordings yet.\nArm HomeGuard to start monitoring.")
        ...
        n = len(files)
        self.count_lbl.setText(f"{n} recording{'s' if n != 1 else ''}")
        cols = 5
    ...
    def _clear_all(self) -> None:
        ...
        reply = QMessageBox.question(
            self,
            "Clear All",
            f"Permanently delete all {len(files)} recordings?",
            QMessageBox.Yes | QMessageBox.No,
        )
        if reply == QMessageBox.Yes:
            for fp in files:
